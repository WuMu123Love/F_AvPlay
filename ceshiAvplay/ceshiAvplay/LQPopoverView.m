...
@interface LQPopoverView ()<UITableViewDataSource, UITableViewDelegate>
...
-(CGRect)getViewFrame
{
    ...
    if ([self.titleArray count] * ROW_HEIGHT + SPACE + kArrowHeight>([UIScreen mainScreen].bounds.size.height*0.6)) {
        frame.size.height = [UIScreen mainScreen].bounds.size.height*0.6;
    }else{
        frame.size.height = [self.titleArray count] * ROW_HEIGHT + SPACE + kArrowHeight;
    }
    for (NSString *title in self.titleArray) {
        NSString *realTitle = title;
        CGFloat width =  [realTitle sizeWithFont:TITLE_FONT constrainedToSize:CGSizeMake(300, 100) lineBreakMode:NSLineBreakByCharWrapping].width;
